...
def render_deferred_section_gate(
    ...
    *,
    ...
    return deferred_rendering.render_section_gate(
        st,
        st.session_state,
        section_id,
...
def cached_league_context(
    ...
    *,
    ...
    # First-useful / Game Plan / Trade / Waivers paths omit league intelligence.
    # Do not enter cached_league_core_context on that path — it always builds the
    # full intelligence frame (and hashes the player table) even when the payload
    # later discards it.
    core_context: dict = {}
    if include_intelligence:
        with performance.time_block("league_context_core", category="analysis"):
            core_context = cached_league_core_context(
                df_players,
                league_id,
                score_field=score_field,
        ...
        with performance.time_block("league_context_summary", category="analysis"):
            league_summary = cached_league_summary(
                df_players,
                league_id,
                score_field=score_field,
                lineup_settings=lineup_settings,
    ...
    with performance.time_block("league_context_shell", category="analysis"):
...
def render_team_comparison_board(df_display: pd.DataFrame):
    active_context = st.session_state.get("active_league_context", {}) or {}
    return league_workspace_ui.render_team_comparison_board(
        df_display,
        team_tap_markup=_team_tap_markup,
        render_team_card_tap_grid=_render_team_card_tap_grid,
        open_league_team_from_tap=_open_league_team_from_tap,
...
def main():
    ...
    if current_page == "gm_targets":
        ...
        mapped_ids = (
            roster_player_map.get(str(my_roster_id), ())
            if my_roster_id is not None
            else ()
        )
        if mapped_ids:
            my_ids = {str(pid) for pid in mapped_ids}
    ...
    if current_page == "my_team":
        ...
        if startup_team_blocked:
            ...
            if not player_ids:
                ...
                team_row = shell_chrome_schema.select_roster_row(df_display, my_roster_id)
                intel_row = shell_chrome_schema.select_roster_row(df_intel, my_roster_id)
                # Posture Outlook reads archetype from intelligence frame when detail ranks omit it.
                if isinstance(team_row, dict):
                    posture_row = dict(team_row)
                elif hasattr(team_row, "to_dict"):
                    posture_row = dict(team_row.to_dict())
                else:
                    posture_row = {}
                intel_map = (
                    intel_row
                    if isinstance(intel_row, dict)
                    else (intel_row.to_dict() if hasattr(intel_row, "to_dict") else {})
                )
                if not _safe_text(posture_row.get("archetype_label")) and _safe_text(
                    intel_map.get("archetype_label")
                ):
                    posture_row["archetype_label"] = intel_map.get("archetype_label")
                    posture_row["archetype_explanation"] = intel_map.get(
                        "archetype_explanation"
                    )
            ...
            if league_summary_df.empty:
                ...
                if league_section == "Rankings":
                    ...
                    if league_intel_detail is None:
                        st.info(
                            "Team comparison needs League Insights detail. "
                            "Summary/shell ranks alone are not enough for this board."
                        )
                    else:
                        render_section_header(
                            "Team comparison",
                            kicker="League structure",
                        ...
                        if render_deferred_section_gate(
                            scoring_section_id,
                            button_label="Load scoring and health detail",
                            note="Position scores, age, and health stay collapsed until you need them.",
                        ):
                            detail_rows = []
                            for _, row in league_intel_detail.reset_index(drop=True).iterrows():
                                detail_rows.append(
                                    dense_list_primitives.dense_row_html(
                                        ...
                                        identity_html=dense_list_primitives.dense_identity_html(
                                            ...
                                            secondary=_safe_text(row.get("owner_name")),
                                        ),
                                        ...
                                        trail_html=dense_list_primitives.dense_trail_html(
                                            meta_html=dense_list_primitives.dense_meta_html(
                                                f"QB {_format_score(row.get('qb_score'))}",
                                                f"RB {_format_score(row.get('rb_score'))}",
                                                f"WR {_format_score(row.get('wr_score'))}",
                                                f"TE {_format_score(row.get('te_score'))}",
                                                f"Age {_safe_text(row.get('avg_age'), '—')}",
                                                _safe_text(row.get("health_flag")),
                                            )
                                        ),
                                        density="compact",
                                    )
                            ...
                            render_html_fragment(
                                "<div class='dg-ranked-board' aria-label='Scoring and health detail'>"
                                + "".join(detail_rows)
                                + "</div>"
                            )
