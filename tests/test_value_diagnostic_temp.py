...
def test_temporary_value_diagnostic():
    ...
    try:
        rows = connection.execute(
            """
            SELECT name, player_id, position, team, age, value, fantasycalc_value,
                   market_score, score, dynasty_score, value_score, rebuild_score,
                   player_tier, opportunity_label
            FROM players
            WHERE name IN ('Jalen Hurts', 'Ricky Pearsall')
            ORDER BY name
            """
        ).fetchall()
    finally:
