...
.player-dossier-rank-strip{align-items:baseline;display:flex;flex-wrap:wrap;gap:var(--space-sm);justify-content:space-between;margin:var(--space-sm) 0;padding:0 var(--space-md)}
.player-dossier-rank-strip strong{color:var(--color-text-primary);font-size:var(--font-size-body);letter-spacing:var(--letter-spacing-badge)}
...
.player-dossier-resume-meta{align-items:center;border-bottom:var(--border-width-default) solid var(--color-border);display:grid;gap:var(--space-xs);grid-template-columns:auto 1fr auto;padding:var(--space-sm) var(--space-md)}
...
.player-dossier-career-empty,.player-dossier-career-group,.player-dossier-context-summary,.player-dossier-context-note{padding:var(--space-sm) var(--space-md)}
.player-dossier-context-note{border-top:var(--border-width-default) solid var(--color-border);color:var(--color-text-muted)}
.player-dossier-career-group h4{color:var(--color-text-primary);font-size:var(--font-size-body);margin:0 0 var(--space-xs)}
.player-dossier-career-group ul{color:var(--color-text-secondary);margin:0;padding-left:var(--space-lg)}
...
@media (max-width: 900px){.player-dossier-snapshot-grid{grid-template-columns:repeat(2, minmax(0, 1fr))}
.player-dossier-executive-grid{grid-template-columns:repeat(2, minmax(0, 1fr))}
...
.player-dossier-section-heading,.player-dossier-snapshot-title,.player-dossier-snapshot-metric,.player-dossier-decision,.player-dossier-career-empty,.player-dossier-career-group,.player-dossier-context-summary,.player-dossier-context-note{padding-left:var(--space-sm);padding-right:var(--space-sm)}
.player-dossier-executive-metric,.player-dossier-resume-meta,.player-dossier-achievement,.player-dossier-timeline-season{padding-left:var(--space-sm);padding-right:var(--space-sm)}
}
@media (prefers-reduced-motion: reduce){.player-dossier-snapshot *,.player-dossier-executive *,.player-dossier-career *,.player-dossier-recommendation-context *{animation:none !important;transition:none !important}
}
"""
