...
.dg-ranked-board{display:grid;gap:var(--space-xs);margin:0 0 var(--space-md);max-width:min(100%,90rem)}
...
.dg-ranked-row--top{border-color:var(--border-accent)}
.dg-ranked-rank{align-items:center;background:var(--surface-raised);border:var(--border-width-default) solid var(--border-standard);border-radius:var(--radius-control);color:var(--text-primary);display:flex;font:var(--font-weight-display) var(--font-size-badge)/1 var(--font-family-sans);height:1.75rem;justify-content:center;min-width:1.75rem;padding:0 var(--space-2xs)}
.dg-ranked-identity{align-items:center;display:flex;gap:var(--space-xs);min-width:0}
.dg-ranked-copy{display:grid;gap:0;min-width:0}
.dg-ranked-team,.dg-dense-identity__primary{color:var(--text-primary);font:var(--type-card-title);overflow-wrap:anywhere}
.dg-ranked-owner,.dg-dense-identity__secondary{color:var(--text-muted);font:var(--type-supporting-metadata);margin-top:1px}
