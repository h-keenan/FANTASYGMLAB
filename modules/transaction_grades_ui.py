...
def recap_grade_strip_html(report: Mapping[str, Any] | None) -> str:
    if not isinstance(report, Mapping) or report.get("pending"):
        return ""
    kind = _text(report.get("kind"))
    if kind == "trade":
        chips = []
        for side in report.get("sides") or []:
            if not isinstance(side, Mapping) or _text(side.get("letter")).casefold() == "pending":
                continue
            chips.append(
        ...
        if not chips:
            return ""
        return "<div class='dg-recap-grades'><span>Early grades</span>" + "".join(chips) + "</div>"
    letter = _text(report.get("letter"))
    if not letter or letter.casefold() == "pending":
        return ""
    return (
        "<div class='dg-recap-grades'><span>Pickup grade</span>"
