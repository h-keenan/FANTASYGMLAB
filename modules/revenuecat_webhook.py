"""Backend-only RevenueCat webhook -> Supabase profile entitlement sync.

Mirrors modules/stripe_webhook.py's pattern (own Supabase service-role PATCH,
own event-id de-dupe cache) so the two billing webhooks stay fully
independent and a bug in one can't affect the other.
"""
...
from modules import revenuecat_billing


class RevenueCatWebhookUpdateError(RuntimeError):
    pass


# Process-local RevenueCat event_id cache. Duplicate deliveries within the TTL
# skip a second Supabase PATCH; entitlement semantics remain idempotent either
# way since the entitlement value is always re-derived from RevenueCat's
# current state, never from the event payload itself.
...
def build_profile_entitlement_payload(action: dict[str, str]) -> dict[str, str]:
    ...
    if entitlement and entitlement not in {"free", "premium"}:
        raise RevenueCatWebhookUpdateError("RevenueCat event did not produce a supported entitlement update.")
    ...
    optional_fields = {
        "revenuecat_app_user_id": action.get("user_id"),
        "revenuecat_event_id": action.get("event_id"),
        "revenuecat_event_type": action.get("event_type"),
        "revenuecat_environment": action.get("environment"),
    }
    for key, value in optional_fields.items():
        clean_value = _safe_text(value)
        if clean_value:
            payload[key] = clean_value
    event_created_ms = _safe_text(action.get("event_created_ms"))
    if event_created_ms:
        try:
            payload["revenuecat_event_created_at"] = datetime.fromtimestamp(
                int(event_created_ms) / 1000, tz=timezone.utc
            ).isoformat()
        except (TypeError, ValueError, OverflowError) as exc:
            raise RevenueCatWebhookUpdateError("RevenueCat event timestamp is invalid.") from exc
    if "entitlement" not in payload and len(payload) == 1:
        # Only timestamp — treat as no-op skip rather than a failed update.
        raise RevenueCatWebhookUpdateError("noop")
    return payload
...
def _durable_event_ordering_params(payload: dict[str, str]) -> dict[str, str]:
    ...
    Mirrors modules/stripe_webhook.py's guard: at an equal timestamp, only a
    distinct revocation may advance state; an ambiguous grant fails closed.
    """

    created_at = _safe_text(payload.get("revenuecat_event_created_at"))
    event_id = _safe_text(payload.get("revenuecat_event_id"))
    if not created_at:
        return {}
    identity_guard = (
        f"or(revenuecat_event_id.is.null,revenuecat_event_id.neq.{event_id})"
        if event_id
        else ""
    )
    clauses = [
        (
            f"and(revenuecat_event_created_at.is.null,{identity_guard})"
            if identity_guard
            else "revenuecat_event_created_at.is.null"
        ),
        (
            f"and(revenuecat_event_created_at.lt.{created_at},{identity_guard})"
            if identity_guard
            else f"revenuecat_event_created_at.lt.{created_at}"
        ),
    ]
    if _safe_text(payload.get("entitlement")).casefold() == "free" and event_id:
        clauses.append(
            "and("
            f"revenuecat_event_created_at.eq.{created_at},"
            "or(revenuecat_event_id.is.null,"
            f"revenuecat_event_id.neq.{event_id})"
            ")"
        )
...
def update_profile_entitlement(
    *,
    ...
    if not user_id:
        return False, "RevenueCat event did not include a Supabase user id."
    try:
        payload = build_profile_entitlement_payload(action)
    except RevenueCatWebhookUpdateError as exc:
        if str(exc) == "noop":
            return True, ""
    ...
    if response.status_code >= 400:
        ...
        if "column" in lowered and any(
            ...
            return False, (
                ...
                "Run docs/supabase_revenuecat_billing.sql before enabling the RevenueCat webhook."
            )
        return False, "Supabase entitlement update failed."
    return True, ""


def process_verified_revenuecat_webhook(
    *,
    payload: bytes | str,
    authorization_header: str | None,
    revenuecat_config: revenuecat_billing.RevenueCatBillingConfig,
    supabase_config: SupabaseWebhookConfig,
    request_session: Any = requests,
) -> dict[str, Any]:
    action = revenuecat_billing.handle_revenuecat_webhook(
        payload,
