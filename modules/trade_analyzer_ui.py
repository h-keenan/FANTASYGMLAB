...
def _render_side_browser(
    *,
    ...
    query = st.text_input(
        "Filter this roster",
        key=search_key,
        placeholder="Filter name, team, or pick year",
        autocomplete="off",
        label_visibility="collapsed",
    )
    catalog = assembly.catalog_for_side(st.session_state, side=side)
    selected = analyzer_builder.package_identities(
...
def render_trade_analyzer_assembly(
    *,
    ...
    """Stacked You receive / You send roster pickers. Analyze stays outside."""
    ...
    def _trade_analyzer_assembly() -> None:
        ...
        st.markdown("<div class='toa-builder-marker toa-assembly-stack'></div>", unsafe_allow_html=True)
        receive_caption = (
            f"From {partner_name}" if partner_name else "From the team that sent this offer"
        )
        _render_side_browser(
            ...
            empty_text="No assets selected to receive.",
        ...
        _render_side_browser(
            ...
            empty_text="No assets selected to send.",
            partner_roster_id=str(partner_roster_id or ""),
            my_roster_id=str(my_roster_id or ""),
