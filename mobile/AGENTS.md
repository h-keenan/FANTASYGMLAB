...
Before touching any color/token in `theme.ts` (mobile) or `modules/design_tokens.py` (web), read `mobile/UI_COLOR_SYSTEM_AUDIT.md`. It requires explicit dark AND light values for every semantic token (not one palette with the background swapped), one consistent meaning per color family across both themes, and forbids page-specific color patches — if a color fails in more than one place, fix the shared token.
